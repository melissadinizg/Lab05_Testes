...
public class ExercicioTeste {
	...
	public void testExercicio() {
	    ...
	    try {
		 	   exc2 = new Exercicio("livre", 0);
		 	   fail("Deveria ter lancado excecao de nome");
		 	} catch ( Exception exp ) {
		 	    assertEquals("Mensagem de excecao capturada.","Calorias por hora do exercicio nao pode ser zero ou menor do que zero", exp.getMessage());
		 	}
	    
	    try {
		 	   exc2 = new Exercicio("livre", -10);
		 	   fail("Deveria ter lancado excecao de nome");
		 	} catch ( Exception exp ) {
		 	    assertEquals("Mensagem de excecao capturada.","Calorias por hora do exercicio nao pode ser zero ou menor do que zero", exp.getMessage());
		 	}
